...
def create_intrinsic_sed(teff,logg,lum,Z,dist):
    ...
    kurucz_sed = kurucz.generate_stellar_spectrum(*ap) # from pystellibs
    
    # wavelengths must be determined based on ap
    sed=pd.DataFrame({'wavelength':kurucz._wavelength,'flux':kurucz_sed/fluxconv}) 
    
    #Return intrinsic SED of one star at a given distance
    #Returns wavelength in angstroms, flux in erg/cm2/s/AA
    return sed

def create_apparent_sed(teff1,teff2,logg1,logg2,lum1,lum2,Z,dist,A_V,R_V=3.1):
    ...
    sed_1=create_intrinsic_sed(teff1,logg1,lum1,Z,dist)
    sed_2=create_intrinsic_sed(teff2,logg2,lum2,Z,dist)
